...
public class EstilosController {
	...
	@GetMapping
	public ModelAndView pesquisar(Estilo estilo,
			BindingResult result, @PageableDefault(size = 4) Pageable pageable, HttpServletRequest http) {
		ModelAndView mv = new ModelAndView("estilo/PesquisaEstilo");
		PageWrapper<Estilo> pagina = new PageWrapper<>(estilorepository.filtrar(estilo, pageable), http);
	...
	public ModelAndView editar(@PathVariable("codigo") Estilo estilo) {
		...
		mv.addObject(this.cadastroEstiloService.buscaPorCodigo(estilo.getCodigo()));
		return mv;
	}
